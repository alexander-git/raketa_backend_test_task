...
1. В задании не сказано какого вида должны быть эндпоинты e методов корзины. Поэтому сделал для чего-то
   вроде GET /cart POST и /cart/add-item. Т.е. без id корзины в uri. Если бы тут предполагалось, что это это
   REST API, то для работы с такими uri необходимо присылать данные нужные для идентификации пользователя 
   при каждом запросе. Для упрощения сделан пример с Basic-авторизацией.
2. CartRepository зависит от RedisConnector. Предполагается его внедрение с помощью метода create
